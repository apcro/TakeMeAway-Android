...
public class SwipeHelper implements View.OnTouchListener {
    ...
    private void checkViewPosition() {
        ...
        float parentTopThird = mSwipeStack.getHeight() / 3f;
        float parentBottomThird = parentTopThird * 2;
        ...
        } else if (viewCenterVertical < parentTopThird && mSwipeStack.getAllowedSwipeDirections() != SwipeStack.SWIPE_DIRECTION_ONLY_UP) {
            swipeViewUp(mAnimationDuration / 2);
        } else if (viewCenterVertical > parentBottomThird && mSwipeStack.getAllowedSwipeDirections() != SwipeStack.SWIPE_DIRECTION_ONLY_DOWN) {
            swipeViewDown(mAnimationDuration / 2);
        } else {
            resetViewPosition();
        }
    }
    ...
    private void swipeViewUp(int duration) {
        Log.d(TAG, "Swiped UP");
        if (!mListenForTouchEvents) return;
        mListenForTouchEvents = false;
        mObservedView.animate().cancel();
        float mObservedViewY = mObservedView.getY();
        float mObservedViewX = mObservedView.getX();
        ...
        mObservedView.animate()
                ...
                .setListener(new AnimationUtils.AnimationEndListener() {
                    @Override
                    public void onAnimationEnd(Animator animation) {
                        mSwipeStack.onViewSwipedUp();
                    }
                });
    }

    private void swipeViewDown(int duration) {
